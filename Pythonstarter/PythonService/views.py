...
def Itenary(request):
    try:
    	if request.method == "GET":
	        ...
	        queryset2 = SearchDetails.objects.raw("select * from search_details where search_id= %s and total_price <= all(select total_price from search_details where search_id= %s )",[str(temp),str(temp)])
